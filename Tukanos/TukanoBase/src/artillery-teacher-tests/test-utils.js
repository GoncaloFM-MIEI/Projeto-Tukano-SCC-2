...
function addTestData() {
	...
	if( fs.existsSync( '/shorts'))
		...
		basefile =  'src/artillery-teacher-tests/data/users.csv'
...
function loadData() {
	...
	if( fs.existsSync( '/shorts'))
		...
		basefile =  'src/artillery-teacher-tests/shorts/house.'
	for( i = 1; i <= 50 ; i++) {
...
function saveShorts() {
	...
	if (fs.existsSync( '/shorts'))  {
		...
		basefile1 = 'src/artillery-teacher-tests/shorts/shorts.list'
        basefile2 = 'src/artillery-teacher-tests/shorts/shorts.map'
    }
...
function prepareLoginForAllUsers(requestParams, context, ee, next) {
    if (loginIndex >= registeredUsers.length) {
        throw new Error("No more users to log in.");
    }

    const user = registeredUsers[loginIndex];
    loginIndex += 1;
...
function prepareLoginForAllUsers(requestParams, context, ee, next) {
    ...
    if (!context.vars.loginIndex) {
        context.vars.loginIndex = 0;
    }

   
    const user = registeredUsers[context.vars.loginIndex];
    if (!user) {
        return next(new Error("No more users available for login."));
    }

   
    context.vars.loginIndex += 1;
